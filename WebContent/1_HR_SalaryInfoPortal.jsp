...
<head>
    ...
    <title>Employee Salary Information Portal</title>
    <!-- CSS -->
          ...
          <table>
					<tr>
						<td><form action="" method="post">
							<input type="text" name="q" class="formtext">&nbsp
							<input id="searchbutton" type="submit" value="Search">	
							</form>
						</td>
						<td  style="padding: 10px">
							<a href="1_HR_salary-form.jsp"><button>Add New Record</button></a>
						</td>
...
%> 
                ...
                <tr>
                    ...
                    <th class="tg-ggay">NIC</th>
                    <th class="tg-ggay">Basic Salary</th>
                    <th class="tg-ggay">Bonus</th>
                    <th class="tg-ggay">Commission</th>
                    <th class="tg-ggay">Deduction</th>
                    <th class="tg-ggay">OT Rate</th>
                    <th class="tg-ggay">OT Hours</th>
                    <th class="tg-ggay">Net Salary</th>
...
if(query !=null){
	...
	sql = "select e.empnic, s.nic, e.fname, e.lname, s.basicSal, s.bonus, s.decrement, s.otrate, s.othrs, s.commission,s.total from blueLite.salary s inner join blueLite.employee e on e.empnic = s.nic  where s.nic like '%"+query+"%'";
	
	 
}else{
	
sql ="select e.empnic, s.nic, e.fname, e.lname, s.basicSal, s.bonus, s.decrement, s.otrate, s.othrs, s.commission,s.total from blueLite.salary s inner join blueLite.employee e on e.empnic = s.nic";
                ...
                <tr>
                  <td><%=rs.getString("fname")%> <%=rs.getString("lname")%></td>
                   <td ><%=rs.getString("nic")%> </td>
                    ...
                    <td ><%=rs.getDouble("basicSal")%></td>
                     <td><%=rs.getDouble("bonus")%></td>
                     <td ><%=rs.getDouble("decrement")%></td>
                     <td ><%=rs.getDouble("otrate")%></td>
                     <td ><%=rs.getDouble("othrs")%></td>
                     <td ><%=rs.getDouble("commission")%></td>
                     <td ><%=rs.getDouble("total")%></td>
                   
                    
                    <td class="tg-i0kf"><a href="1_HR_EditSalaryInfo.jsp?edit_id=<%=rs.getString("nic")%>">Edit</a></td>
                    <td class="tg-i0kf"> <a href="1_HR_DeleteSalary.jsp?delete_id=<%=rs.getString("nic")%>">Delete</a></td>
