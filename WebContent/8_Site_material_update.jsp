...
<script type="text/javascript"> 
        function validate(form)
        {
        	...
        	//material Name field validation
            if(form.materialName.value == ""){
              errors.push("Error: Material Name Field cannot be left empty!");         
            }
        
        	//quantity field validation
            if(form.quantity.value == ""){
            	errors.push("Error: Quantity Field cannot be left empty!");    
            }
            else if(!num.test(form.estimatedCost.value)){
            	errors.push("Error: Quantity contains invalid characters! \n**PLEASE ENTER NUMERICALS ONLY**");
            }
        ...
        <%
            if(request.getParameter("update_id")!=null) 
            {
                ...
                try
                {
                    ...
                    pstmt = conn.prepareStatement("select * from material where materialID=?"); 
                    pstmt.setInt(1,id);
         ...
         <form method="POST" action="Site_MaterialUpdateController" style="float:center" onsubmit="return validate(this);">
            <input class="formtext" name="materialName" value="<%= rs.getString("materialName") %>" placeholder="Material Name" type="text">
            <br><br>
            <%-- select class="formtext" name="materialName" required="required" style="width:200px" value="<%= rs.getString("materialName") %>">
           		<option value="<%= rs.getString("Fired Bricks") %>">Fired Bricks</option>
           		<option value="<%= rs.getString("Clay Blocks") %>">Clay Blocks</option>
           		<option value="<%= rs.getString("Cement") %>">Cement</option>
           		<option value="<%= rs.getString("Fabric") %>">Fabric</option>
           		<option value="<%= rs.getString("Foam") %>">Foam</option>
           		<option value="<%= rs.getString("Glass") %>">Glass</option>
           		<option value="<%= rs.getString("Metal") %>">Metal</option>
           		<option value="<%= rs.getString("Plastic") %>">Plastic</option>
           		<option value="<%= rs.getString("Ceramic") %>">Ceramic</option>
            </select> --%>
            <input class="formtext" name="quantity" required="required" value="<%= rs.getDouble("quantity") %>" placeholder= "Quantity " type="number">
            <br><br>
            <input class="formbutton" type="submit" name="update" value="UPDATE">
            <input type="hidden" name="hidden_id" value="<%=rs.getInt("materialID")%>">
