...
if(typeof Muse == "undefined") window.Muse = {}; window.Muse.assets = {"required":["museutils.js", "museconfig.js", "jquery.musepolyfill.bgsize.js", "jquery.musemenu.js", "jquery.watch.js", "webpro.js", "musewpdisclosure.js", "require.js", "new-finance-entry.css"], "outOfDate":[]};
  ...
  <![endif]-->
	...
	function check(form){
    	...
    	var quo = document.getElementById("quota").value;
    	
    	if(quan <= quo){
    		alert("Allocated Quota is higher than the AVAILABLE STOCK !");
    	    window.location.href = "4_IL_email.jsp";
    	}
    	else  
    		alert("Raw Material Insertion is a SUCCESS !");
    
    }
    
    function total(form) {
    	var quan = document.getElementById("quantity").value;
    	var cost = document.getElementById("unitprice").value;
  ...
  <div class="museBGSize clearfix borderbox" id="page"><!-- group -->
   <div class="clearfix grpelem" id="pu133"><!-- group -->
    ...
    <div class="clearfix" id="u3499-6"><!-- content -->
     ...
     <p id="u3499-4">Inventory Manager</p>
    </div>
   </div>
        ...
        <%
            if(request.getParameter("update_id")!=null) 
            {
                ...
                try
                {
                    ...
                    pstmt = conn.prepareStatement("select * from raw_material where id=?"); 
                    pstmt.setInt(1,id);
         ...
         <form method="POST" action="IL_RawMaterialUpdateController" style="float:center">
            <input class="formtext" name="type" value="<%= rs.getString("type") %>" placeholder="Material Name" type="text" pattern="^[a-z A-Z]+$" title="Please include characters(a-z A-Z) only!" required>
            <br><br>
            <input class="formtext" name="quantity" value="<%= rs.getDouble("quantity") %>" placeholder= "Quantity" type="number" step="any" min="0" pattern="[0-9]*[.,]?[0-9]+" required>
            <br><br>
            <select id="dropdown" class="formtext" name="unit" value="<%=rs.getString("unit")%>" required>
            ...
            <br><br>
            <input class="formtext" name="unitprice" value="<%= rs.getDouble("unit_cost") %>" placeholder= "Unit Price" type="number" min="0" type="number" step="any" min="0" pattern="[0-9]*[.,]?[0-9]+" required>
            <br><br>
            <input class="formtext" name="quota" value="<%= rs.getDouble("quota") %>" placeholder= "Minimum Quota" type="number" min="0" type="number" step="any" min="0" pattern="[0-9]*[.,]?[0-9]+" required>
            <br><br>
            <input type="hidden" name="hidden_id" value="<%= rs.getInt("id") %>">
            <input type="hidden" name="hidden_total" id="hidden_total" value="<%= rs.getDouble("total") %>">
